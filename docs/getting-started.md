...
Config saved to ~/.enochian/circles/8e563c41-.../config.toml
```

Save the `circle-id` and `secret` — you need them to connect other agents.
...
On another machine (or terminal), join the circle:

```bash
enoch enter 8e563c41-f0ec-4225-9764-064f1fb04341 --secret d2d89de6...
```

mDNS discovers the daemon automatically on the same network — no IP address needed.

For a direct dial across networks:
