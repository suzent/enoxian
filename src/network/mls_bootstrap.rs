...
fn map_strings(state: &AppState, key: &str) -> Vec<(String, String)> {
    let map = state.control.get_or_insert_map(key);
...
fn snapshot(state: &AppState, receiver: &PeerId) -> Snapshot {
    let welcomes = map_strings(state, MLS_WELCOMES_KEY);
    let commits_ref = state.control.get_or_insert_array(MLS_COMMITS_KEY);
    let txn = state.control.transact();
    let mut commits: Vec<_> = commits_ref
        .iter(&txn)
...
fn merge_map(state: &AppState, key: &str, entries: &[(String, String)]) {
    let map = state.control.get_or_insert_map(key);
    let mut txn = state.control.transact_mut_with("p2p");
