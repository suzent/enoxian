...
async fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        ...
        AgentCommands::Invite(args) => {
            let configs = enoxian::config::load_all()?;
            let cfg = enoxian::resolve::resolve(&args.circle, &configs)
                .map_err(|_| anyhow::anyhow!("circle '{}' not found — run `enox circles` to list known circles", args.circle))?;
        ...
        AgentCommands::Update { dev, src, no_pull } => enoxian::commands::update::run(dev, src, no_pull).await,
        ...
        cmd => {
            ...
            match &cmd {
                AgentCommands::Disable => {
                    return enoxian::commands::disable::run(&client, &root, cli.circle.as_deref()).await;
                }
                AgentCommands::Enable => {
                    return enoxian::commands::enable::run(&client, &root, cli.circle.as_deref()).await;
                }
                AgentCommands::Leave { yes } => {
                    return enoxian::commands::leave::run(&client, &root, cli.circle.as_deref(), *yes).await;
                }
                _ => {}
            }
            ...
            match cmd {
                ...
                AgentCommands::Who =>
                    enoxian::commands::who::run(&client, &base, cli.json).await,
                ...
                AgentCommands::TaskCreate { title, description } =>
                    enoxian::commands::tasks::create(&client, &base, title, description, cli.json).await,
                ...
                AgentCommands::Watch =>
                    enoxian::commands::watch::run(&client, &base).await,
                AgentCommands::Member(args) =>
                    enoxian::commands::member::run(&client, &root, cli.circle.as_deref(), args.action, cli.json).await,
