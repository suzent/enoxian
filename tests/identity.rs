...
fn circle_keypair_is_deterministic() {
    ...
    let pid1 = d.derive_circle_keypair("circle-abc").unwrap().public().to_peer_id();
    let pid2 = d.derive_circle_keypair("circle-abc").unwrap().public().to_peer_id();
    assert_eq!(pid1, pid2, "same device + same circle_id must always produce the same peer ID");
}

#[test]
fn different_circles_produce_different_peer_ids() {
    let d = DeviceIdentity::generate("test".to_string());
    let pid_a = d.derive_circle_keypair("circle-alpha").unwrap().public().to_peer_id();
    let pid_b = d.derive_circle_keypair("circle-beta").unwrap().public().to_peer_id();
    assert_ne!(pid_a, pid_b, "different circle IDs must yield different peer IDs");
}
...
fn different_devices_produce_different_peer_ids_for_same_circle() {
    ...
    let pid1 = d1.derive_circle_keypair("shared-circle").unwrap().public().to_peer_id();
    let pid2 = d2.derive_circle_keypair("shared-circle").unwrap().public().to_peer_id();
    assert_ne!(pid1, pid2, "different devices must produce different peer IDs for the same circle");
}
...
fn device_keypair_differs_from_circle_keypair() {
    ...
    let circle_pid = d.derive_circle_keypair("some-circle").unwrap().public().to_peer_id();
...
fn invalid_mnemonic_is_rejected() {
    assert!(UserIdentity::from_mnemonic("not a valid mnemonic phrase here", "x".to_string()).is_err());
}
...
fn attestation_signature_is_64_bytes() {
    ...
    let attestation = user.attest_device(&device_pubkey, &device.device_label).unwrap();
    // Ed25519 signature = 64 bytes → 128 hex chars
    assert_eq!(attestation.len(), 128, "attestation must be a 64-byte Ed25519 signature");
}
...
fn different_users_produce_different_attestations_for_same_device() {
    ...
    let att1 = u1.attest_device(&device_pubkey, &device.device_label).unwrap();
    let att2 = u2.attest_device(&device_pubkey, &device.device_label).unwrap();
    assert_ne!(att1, att2);
}
...
fn peer_id_survives_handle_update() {
    ...
    let pid_before = d.derive_circle_keypair("circle-x").unwrap().public().to_peer_id();

    d.set_user_handle("suzy".to_string());
    let pid_after = d.derive_circle_keypair("circle-x").unwrap().public().to_peer_id();

    assert_eq!(pid_before, pid_after, "changing user handle must not change the circle keypair");
}
...
fn many_circles_all_produce_distinct_peer_ids() {
    ...
    for i in 0..peer_ids.len() {
        for j in (i + 1)..peer_ids.len() {
            assert_ne!(peer_ids[i], peer_ids[j], "circles {} and {} collided", circles[i], circles[j]);
        }
    }
}
